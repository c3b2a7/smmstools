...
function build() {
  ...
  if [[ "$TARGET" == *"-linux-"* ]]; then
    PKG_NAME="smmstools-v${VERSION}.${TARGET}.tar.gz"
    ...
    if [[ "${use_upx}" = true ]]; then
      # Enable upx for MIPS.
      $upx smmstools #>/dev/null
    fi

    echo "* Packaging GZ in ${PKG_PATH} ..."
    tar -czf ${PKG_PATH} "smmstools"
    ...
    PKG_NAME="smmstools-v${VERSION}.${TARGET}.zip"
    PKG_PATH="${PKG_DIR}/${PKG_NAME}"

    echo "* Packaging ZIP in ${PKG_PATH} ..."
    cd ${RELEASE_DIR}
    zip ${PKG_PATH} "smmstools.exe"
